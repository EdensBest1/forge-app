...
assert.match(html, /id="manufacturingLeadSourceCrm" data-operator-only/);
assert.match(html, /id="manufacturingRfqList" data-operator-only/);
...
assert.match(app, /privateManufacturingView\s*=\s*canRenderPrivateOperatorData\(\)/);
assert.match(app, /Supplier identities and contact details stay private\./);
assert.match(app, /supplierLeadList\.innerHTML\s*=\s*privateManufacturingView/);
